...
BEGIN
    ...
    LOOP
        ...
        tbl_name := REPLACE(seq_record.sequence_name, '_id_seq', '');
        ...
        IF EXISTS (
            ...
            -- Get max ID from table (default to 1 for empty tables)
            EXECUTE format('SELECT COALESCE(MAX(id), 1) FROM %I', tbl_name) INTO max_id;
